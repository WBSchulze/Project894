...
paths, count = project.approxSolveDp( pairs, g.edges )
...
try:
    ...
    with warnings.catch_warnings():
        ...
        for iPair in range(len(pairs)):
            if paths[iPair] is None:
                continue
            orderly = [pairs[iPair][0]]
            while len(paths[iPair]):
                nextEdge = [ edge for edge in paths[iPair] if edge[0] == orderly[-1] ]
                nextEdge = nextEdge[0] # Will fail if the solution is broken
                G.get_edge(*nextEdge).attr["color"] = colors[iPair]
                iNext = paths[iPair].index(nextEdge)
                paths[iPair].pop( iNext )
                orderly.append( nextEdge[1] )
            print( f"Path {iPair}:")
            print( "->".join([str(v) for v in orderly]) )


        G.draw( f"approxGraph.png" )
